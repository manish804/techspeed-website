...
export default function SolutionsPage() {
    const [heroRef, isHeroVisible] = useScrollAnimation({ threshold: 0.1, triggerOnce: true })
    ...
    return (
        <div className="pt-20">
            ...
            <section className="bg-gradient-to-br from-[#F5F3FF] to-white py-24 relative overflow-hidden">
                ...
                <div
                    className="absolute -top-16 -right-16 w-64 h-64 rounded-full bg-[#A855F7]/10 blur-2xl"
                    style={{ transform: `translateY(${parallaxOffset * 0.5}px)` }}
                />
                <div className="max-w-[1400px] mx-auto px-8 relative z-10">
                    <div 
                        ref={heroRef}
                        className={`max-w-3xl scroll-animate ${isHeroVisible ? 'visible' : ''}`}
                    >
                ...
                <div className="max-w-[1400px] mx-auto px-8">
                    <div className="space-y-24">
                        {solutions.map((solution, index) => (
                            ...
                            >
                                <AnimatedSolutionIntro index={index} solution={solution} />
                                <AnimatedSolutionBenefits index={index} solution={solution} />
                            </div>
                        ))}
                    </div>
                </div>
...
function AnimatedSolutionIntro({ index, solution }) {
    const [introRef, introVisible] = useScrollAnimation({ threshold: 0.12, triggerOnce: true })
    const { getItemRef, getItemStyle } = useStaggeredAnimation(120)
    return (
        <div ref={introRef} className={`${index % 2 === 1 ? 'lg:order-2' : ''} scroll-animate ${introVisible ? 'visible' : ''}`}>
            ...
            <div className="mb-8">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {solution.features.map((feature, i) => (
                        <li key={feature} ref={getItemRef(i)} style={getItemStyle(i)} className="flex items-start gap-2">
...
function AnimatedSolutionBenefits({ index, solution }) {
    const [benefitsRef, benefitsVisible] = useScrollAnimation({ threshold: 0.12, triggerOnce: true })
    const { getItemRef, getItemStyle } = useStaggeredAnimation(120)
    return (
        <div ref={benefitsRef} className={`${index % 2 === 1 ? 'lg:order-1' : ''} scroll-animate-right ${benefitsVisible ? 'visible' : ''}`}>
            <div className="bg-[#F5F3FF] rounded-2xl p-8 card-elevated rounded-transition">
                ...
                <ul className="space-y-4">
                    {solution.benefits.map((benefit, i) => (
                        <li key={benefit} ref={getItemRef(i)} style={getItemStyle(i)} className="flex items-start gap-3">
                            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[#A855F7] text-white flex items-center justify-center text-sm">✓</span>
